...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		// This shader handles our basic materials without reflections
		...
		// This shader handles our basic specular materials without reflections
		...
		MeshResource::Sptr pacmapMesh = ResourceManager::CreateAsset<MeshResource>("pacmap.obj");

		// Load in some textures
		Texture2D::Sptr    pacmanTex = ResourceManager::CreateAsset<Texture2D>("textures/pacman.png");
		Texture2D::Sptr    pacmapTex = ResourceManager::CreateAsset<Texture2D>("textures/cakes.png");
		...
		// Pacman Level Material
		Material::Sptr pacmapMaterial = ResourceManager::CreateAsset<Material>(toonShader);
		{
			pacmapMaterial->Name = "Toon";
			pacmapMaterial->Set("u_Material.Diffuse", pacmapTex);
			pacmapMaterial->Set("s_ToonTerm", toonLut);
			pacmapMaterial->Set("u_Material.Shininess", 0.1f);
			pacmapMaterial->Set("u_Material.Steps", 3);
		}

		// Pacman Material
		Material::Sptr pacmanMaterial = ResourceManager::CreateAsset<Material>(toonShader);
		{
			pacmanMaterial->Name = "Toon";
			pacmanMaterial->Set("u_Material.Diffuse", pacmanTex);
			pacmanMaterial->Set("s_ToonTerm", toonLut);
			pacmanMaterial->Set("u_Material.Shininess", 0.1f);
			pacmanMaterial->Set("u_Material.Steps", 3);
		}
		...
		GameObject::Sptr map = scene->CreateGameObject("Map");
		{
			...
			renderer->SetMesh(pacmapMesh);
			renderer->SetMaterial(pacmapMaterial);
		}

		GameObject::Sptr pac = scene->CreateGameObject("Pacman");
		{
			// Create and attach a RenderComponent to the object to draw our mesh
			RenderComponent::Sptr renderer = pac->Add<RenderComponent>();
			renderer->SetMesh(pacmanMesh);
			renderer->SetMaterial(pacmanMaterial);
		}
